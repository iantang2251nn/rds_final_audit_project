...
The implementation is accurate on the metric the competition rewarded (Overall AUC $0.97368$, final score $0.94464$). It is not robust or fair under either of the two stakeholder-grounded readings of group fairness we tested (predictive parity and error-rate balance). Pillar~1 flags five over-flag and two under-flag identities, with error-direction falling on disjoint groups (\S5.1). Pillar~3 confirms substantial dispersion in PPV ($[0.73, 0.84]$), FPR ($[0.012, 0.060]$), and selection rate (\S5.2), and Chouldechova's impossibility result rules out simultaneous predictive parity and error-rate balance at the observed base rates---forcing a stakeholder choice the developers made implicitly through their training objective. Pillar~2's counterfactual gaps fall on the same identities Pillar~1 flagged, providing independent corroboration that the bias is real rather than a test-set artifact.
