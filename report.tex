...
The system audited here is reproduced from sakami0000's publicly available code \citep{sakami2019jigsaw}. We audit the recreated BERT-large + LSTM-GRU ensemble, not the original five-component F.H.S.D.Y.\ ensemble, and are explicit about the recreation's scope and its fairness implications in \S4.1.
...
\caption{Visual rendering of Table~\ref{tab:bias-aucs}: Subgroup, BPSN, and BNSP AUCs per evaluated identity. Low BPSN on \texttt{white} and \texttt{black} (race-mention over-flagging) and low BNSP on \texttt{christian} (under-flagging of toxicity targeting the group) drive the direction-of-error split. \texttt{muslim} has both metrics lowered; the BPSN-vs-BNSP rule classifies it as over-flagged.}
\end{figure}
...
\paragraph{Methodology.} Expand per-identity evaluation beyond the nine sample-size-cutoff columns to all twenty-four, with confidence bounds for smaller-sample identities (\S5.3.2). Augment the bias-aware AUC objective with a Module~1 metric (e.g., per-group PPV or FPR variance) at training time, so the metric's normative defaults are not inherited unexamined \citep{raji2020closing}. Run counterfactual substitution tests with the GPT-2 perplexity filter of \S5.3.1 inside validation, not as a post-hoc audit. If structural diversity is the fairness mechanism the original F.H.S.D.Y.\ ensemble exploited, restore its missing components before further blend-weight tuning---the BERT-dominated blend here ($w_{\text{BERT}} = 0.993$) may amplify a single component's inductive bias rather than averaging across components, as Pillar~2's BERT-vs-LSTM divergence suggests.
